...
class JsonRpc2Validator(object):
    ...
    def validate(self, dataStr):
        ...
        if data[JsonRpc2Keys.KEY_JSONRPC] != '2.0':
            raise JsonRpc2ComplianceException( "Data must be version 2.0. %s value is [%s]"%(JsonRpc2Keys.KEY_JSONRPC, data[JsonRpc2Keys.KEY_JSONRPC]) )
    ...
    def validateRequest(self, data):
        if JsonRpc2Keys.KEY_PARAMS in data:
            if type(data[JsonRpc2Keys.KEY_PARAMS]) not in [dict, list]:
                raise JsonRpc2ComplianceException( "%s must be a dictionary or a list."%(JsonRpc2Keys.KEY_PARAMS) )
